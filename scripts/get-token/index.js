...
main().then(
  ...
  (error) => logger.error(pe.render(error))
);
...
async function main() {
  ...
  if (!EMAIL || !PASSWORD) {
    throw new Error("Bad command line arguments; both email and password should be provided");
  }
  ...
  if (!isLoggedIn) {
    ...
    // typing in email
    await safeType(popup, "input[type=email]", EMAIL);

    // submiting email
    await safeClick(popup, "input[type=submit]");

    // typing in password
    await safeType(popup, "input[type=password]", PASSWORD);
